...
def get_plant_day_hpv(hpv_dict, now):
    """
    Calculates the HPV for the day by dividing shift manhours by the claims. If
    no claims, HPV is set to 0 to avoid a DivisionByZero error.
    ...
    :return: Float value - plant day HPV AND Integer - manhours AND Integer -
        claims.
    """
    print("/"*50)
    ...
    if plant_settings.num_of_shifts == 3:
        hpv, mh, claims = get_three_shifts_plant_day_hpv(
            hpv_dict, all_since_start, cur_hpv, cur_mh, cur_claims
        )
    elif plant_settings.num_of_shifts == 2:
        hpv, mh, claims = get_two_shifts_plant_day_hpv(
            hpv_dict, all_since_start, cur_hpv, cur_mh, cur_claims
        )
    else:
...
def get_three_shifts_plant_day_hpv(hpv_dict, all_since_start, cur_hpv, cur_mh,
                                   cur_claims):
    """
    Calculates the HPV for the plant based on 3 shifts active in settings. Gets
    ...
    :param all_since_start: Queryset object - API entries since the start of
        the day
    :param cur_hpv: shift hpv value for the plant - float.
    :param cur_mh: shift manhours value for the plant - integer.
...
def get_three_shifts_plant_day_hpv_2nd_shift(s3, s1, cur_mh, cur_claims):
    """
    Calculates the HPV for the plant based on 3 shifts active in settings and
    "now" being in the second shift. Gets the final snapshots from previous
    shifts to add to the current shift values. HPV is calculated based on
...
def get_two_shifts_plant_day_hpv(hpv_dict, all_since_start, cur_hpv, cur_mh,
                                 cur_claims):
    """
    Calculates the HPV for the plant based on 2 shifts active in settings. Gets
...
def get_last_shift_plant_day_hpv(cur_hpv, cur_mh, cur_claims, last_shift):
    """
    Calculates the HPV for the plant based on only 1 previous shift. Gets the
    final snapshots from the previous shift to add to the current shift values.
    HPV is calculated based on manhour and claim totals for the day so far.

    :param cur_hpv: shift hpv value for the plant - float.
    :param cur_mh: shift manhours value for the plant - integer.
    :param cur_claims: shift claims value - integer.
    :param last_shift: HPVATM model object - API entry for the previous shift.
    :return: Float value - plant day HPV AND Integer - manhours.
    """
