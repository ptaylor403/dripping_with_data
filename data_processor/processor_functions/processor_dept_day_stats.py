...
def get_dept_day_stats(hpv_dict, now, dept):
    """
    Calculates the HPV for the current day by dividing shift manhours by the
    claims. If no claims, HPV is set to 0 to avoid a DivisionByZero error.
    ...
    if plant_settings.num_of_shifts == 3:
        hpv, mh = get_three_shifts_dept_day_hpv(hpv_dict, dept,
                                                all_since_start, cur_hpv,
                                                cur_mh, cur_claims)
    elif plant_settings.num_of_shifts == 2:
        hpv, mh = get_two_shifts_dept_day_hpv(hpv_dict, dept, all_since_start,
                                              cur_hpv, cur_mh, cur_claims)
    else:
...
def get_three_shifts_dept_day_hpv(hpv_dict, dept, all_since_start, cur_hpv, cur_mh, cur_claims):
    """
    Calculates the HPV for the department based on 3 shifts active in settings.
    Gets the final snapshots from previous shifts to add to the current shift
    values. HPV is calculated based on manhour and claim totals for the day so
    far.
    ...
    # If we are in a later shift, add previous shift values to current
        ...
        hpv, mh = get_last_shift_dept_day_hpv(dept, cur_mh, cur_claims,
                                              last_shift)
    elif hpv_dict['shift'] == 2:
        hpv, mh = get_last_two_shifts_dept_day_hpv(dept, all_since_start,
                                                   cur_mh, cur_claims)
...
def get_last_shift_dept_day_hpv(dept, cur_mh, cur_claims, last_shift):
    """
    Calculates the HPV for the department based on only 1 previous shift. Gets
...
def get_last_two_shifts_dept_day_hpv(dept, all_since_start, cur_mh,
                                     cur_claims):
    """
    Returns the total hpv for the day when there are 2 shifts before the
    current time to check. Checks for and handles missing entries for either
    previous shift.

    :param dept: The department to calculate for - string.
    :param all_since_start: HPVATM queryset object - API entries for the
        current day.
    :param cur_mh: shift manhours value for the department - integer.
    :param cur_claims: shift claims value - integer.
    :return: Float value - department day HPV AND Integer for manhours.
    """
    ...
    if s3 is None:
        mh, claims = get_last_two_shifts_dept_day_hpv_missing_shift_three(
            dept, s1, cur_mh, cur_claims
        )
    elif s1 is None:
        mh, claims = get_last_two_shifts_dept_day_hpv_missing_shift_one(
            dept, s3, cur_mh, cur_claims
        )
    else:
        s3_mh = float(getattr(s3, '{}_s_mh'.format(dept)))
        s1_mh = float(getattr(s1, '{}_s_mh'.format(dept)))
        mh = s3_mh + s1_mh + cur_mh
        claims = s3.claims_s + s1.claims_s + cur_claims
    hpv = calc_hpv(mh, claims)
    return hpv, mh


def get_last_two_shifts_dept_day_hpv_missing_shift_three(dept, s1, cur_mh,
                                                         cur_claims):
    """
    Returns the total hpv for the day when there are 2 shifts before the
    ...
    :param dept: The department to calculate for - string.
    :param s1: HPVATM model instance - final entry for today's first shift.
    :param cur_mh: shift manhours value for the department - integer.
    :param cur_claims: shift claims value - integer.
...
def get_last_two_shifts_dept_day_hpv_missing_shift_one(dept, s3, cur_mh,
                                                       cur_claims):
    """
    Returns the total hpv for the day when there are 2 shifts before the
...
def get_two_shifts_dept_day_hpv(hpv_dict, dept, all_since_start, cur_hpv,
                                cur_mh, cur_claims):
    """
    Calculates the HPV for the department based on 2 shifts active in settings.
    Gets the final snapshots from previous shift to add to the current shift
    values. HPV is calculated based on manhour and and claim totals for the day
    so far.
